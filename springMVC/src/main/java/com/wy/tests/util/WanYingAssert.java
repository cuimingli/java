...
public class WanYingAssert {
	...
	public static void throwErrorMessage(Exception e) {
		if (e.getCause() == null) {
			if (e instanceof DefinedMessageException) {
				isTrue(false, ((DefinedMessageException) e).getCode(), e.getMessage());
			} else {
				isTrue(false, e.getMessage());
			}
		} else if (e.getCause().getCause() == null) {
			if (e instanceof DefinedMessageException) {
				isTrue(false, ((DefinedMessageException) e).getCode(), e.getCause().getMessage());
			} else {
				isTrue(false, e.getCause().getMessage());
			}
		} else {
			if (e instanceof DefinedMessageException) {
				isTrue(false, ((DefinedMessageException) e).getCode(), e.getCause().getCause().getMessage());
			} else {
				isTrue(false, e.getCause().getCause().getMessage());
			}
		}
	}
	...
	public static void isTrue(boolean expression, String message) {
		if (!expression) {
			throw new DefinedMessageException("N/A", message);
		}
	}

	public static void isTrue(boolean expression, String code, String message) {
		if (!expression) {
			throw new DefinedMessageException(code, message);
		}
	}
	...
	public static void isInstanceOf(Class<?> type, Object obj, String message) {
		...
		if (!type.isInstance(obj)) {
			throw new DefinedMessageException("N/A",
	...
	public static void isAssignable(Class<?> superType, Class<?> subType, String message) {
		...
		if (subType == null || !superType.isAssignableFrom(subType)) {
			throw new DefinedMessageException("N/A", message + subType + " is not assignable to " + superType);
		}
	}
	...
	public static void isZero(int num, String code, String message) {
		if (num == 0) {
			throw new DefinedMessageException(code, message);
		}
	}
